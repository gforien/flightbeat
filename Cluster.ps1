...
switch($argsString) {
    ...
    "list leaders"          { &{ 1..9 | %{ echo "tc$_ => leader is $((curl -Method GET -Uri http://tc405-112-0$_.insa-lyon.fr:5000/leader).Content)"}}}
    ...
    "start jobs"            { &{ 1..16 | %{ ssh tc$_ "node ~/leader-election/leader.js $_ >> ~/electionlog_$_ 2>&1 &" }}}

    # log files
    "list logs"             { &{ 1..1 | %{ ssh tc$_ "ls electionlog_*" }}}
    "delete logs"           { &{ 1..1 | %{ ssh tc$_ "rm electionlog_*" }}}
    "remove logs"           { &{ 1..1 | %{ ssh tc$_ "rm electionlog_*" }}}
